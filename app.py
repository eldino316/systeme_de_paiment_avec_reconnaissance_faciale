...
# capture d'image pour le dataset
...
def train_classifier(nbr):
    dataset_dir = "I:/Système de Paiment Faciale/dataset"
...
@app.route('/login')
def login():
    mycursor.execute("select ifnull(max(prs_carte) + 1, 101) from prs_info")
    row = mycursor.fetchone()
    nbr = row[0]

    return render_template('login.html', newnbr=int(nbr))

@app.route('/register', methods=['POST'])
def register():

    msg = ''
    carte = request.form['txtcarte']
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form and 'email' in request.form:
        ...
        mycursor.execute('SELECT * FROM prs_info WHERE name = %s', (username,))
        ...
        if account:
            ...
            msg = 'veillez remplir les champs svp!!'
        else:
            carte = request.form['txtcarte']
            numero = request.form['txtnum']
            date = request.form['txtdate']
            prenom = request.form['txtprenom']
            
            mycursor.execute('INSERT INTO prs_info VALUES (%s, %s, %s, %s, %s, %s, %s)', (carte, username, prenom, date, email, password,))
            mydb.commit()
            msg = 'You have successfully registered!'
    elif request.method == 'POST':
        msg = 'veuillez remplir les champs!'

    return redirect(url_for('capture_page', prs=carte))

@app.route('/capture_page/<prs>')
def capture_page(prs):
